...
@model(
    table_description="Kompletterande uppgifter (tidigare medicinsk information) som kan anges eller begäras vid en beställning (MultiLabb analyskatalog)",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'MedInfoID': 'varchar(max)', 'OrderRegistryFileName': 'varchar(max)', 'TextLabel': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Type': 'varchar(max)', 'Unit': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vCodes_MultiLabMedInfo) y
