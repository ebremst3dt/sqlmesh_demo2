...
@model(
    table_description="Alla förändringar av sängplatser och sängar.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'BedDescription': 'varchar(max)', 'BedID': 'varchar(max)', 'BedName': 'varchar(max)', 'CareUnitID': 'varchar(max)', 'Category': 'varchar(max)', 'ChangedDate': 'varchar(max)', 'Comment': 'varchar(max)', 'PatientID': 'varchar(max)', 'SortIndex': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TransactionID': 'varchar(max)', 'ValidUntilDate': 'varchar(max)'},
