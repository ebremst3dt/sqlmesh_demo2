...
@model(
    table_description="Användarfält. I denna tabell lagras endast en rad när en användare har fyllt i ett värde i objektet.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'DocumentID': 'varchar(max)', 'FormTemplateID': 'varchar(max)', 'Name': 'varchar(max)', 'ObjectID': 'varchar(max)', 'ObjectTypeID': 'varchar(max)', 'PatientID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'UserID': 'varchar(max)', 'UserName': 'varchar(max)', 'Version': 'varchar(max)'},
