...
@model(
    table_description="Används för att matcha beställda analyser mot besvarade. Innehåller ofta samma data som Analyses men inte alltid",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'Analysis': 'varchar(max)', 'Dicipline': 'varchar(max)', 'DocumentID': 'varchar(max)', 'HasReply': 'varchar(max)', 'OrderableID': 'varchar(max)', 'PatientID': 'varchar(max)', 'Row': 'varchar(max)', 'Specimen': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
