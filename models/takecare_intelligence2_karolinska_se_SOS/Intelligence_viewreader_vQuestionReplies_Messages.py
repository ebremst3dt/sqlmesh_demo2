...
@model(
    table_description="I denna tabell lagras fråga (alltid första raden) samt ev. svar (alltid andra raden). För de kolumner vars namn börjar på SenderReceiver gäller att om TypeID är 1 eller 3 avses avsändare, och om TypeID är 2 eller 4 avses mottagare.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'ContactInfo': 'varchar(max)', 'DocumentID': 'varchar(max)', 'MessageHeading': 'varchar(max)', 'MessageText': 'varchar(max)', 'MessageTimestampSaved': 'varchar(max)', 'PatientID': 'varchar(max)', 'Row': 'varchar(max)', 'SenderReceiverCareUnitID': 'varchar(max)', 'SenderReceiverUser': 'varchar(max)', 'SenderReceiverUserHSAID': 'varchar(max)', 'SenderReceiverUserID': 'varchar(max)', 'SenderReceiverUserName': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TimestampSent': 'varchar(max)', 'TimestampSigned': 'varchar(max)', 'TypeID': 'varchar(max)', 'Version': 'varchar(max)'},
