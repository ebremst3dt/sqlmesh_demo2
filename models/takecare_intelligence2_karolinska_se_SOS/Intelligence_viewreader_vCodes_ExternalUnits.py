...
@model(
    table_description="Data från register över externa vårdenheter. Användaren definierar, per installation, versionsrader för en vårdenhet, samt vilka kolumner som registret använder. Enhet som tagits bort ur textfilen som laddar registret kommer ej med i tabellen.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CostDepartment': 'varchar(max)', 'EXID': 'varchar(max)', 'ParentEXID': 'varchar(max)', 'Row': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'UnitLevelID': 'varchar(max)', 'UnitName': 'varchar(max)', 'ValidFromDate': 'varchar(max)', 'ValidThroughDate': 'varchar(max)', 'WorkPlaceCode': 'varchar(max)'},
