...
@model(
    table_description="Provtagningstillfällen.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'DocumentID': 'varchar(max)', 'GroupRow': 'varchar(max)', 'HoursSinceLatestDose': 'varchar(max)', 'LID': 'varchar(max)', 'OccasionRow': 'varchar(max)', 'PatientID': 'varchar(max)', 'SamplingDate': 'varchar(max)', 'SamplingTime': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TreatmentTime': 'varchar(max)', 'Unit24HourDose': 'varchar(max)', 'Value24HourDose': 'varchar(max)', 'Version': 'varchar(max)'},
