...
@model(
    table_description="Aktiviteten utförs var n:te dag, en till flera gånger varje dag.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'EndDate': 'varchar(max)', 'EndTime': 'varchar(max)', 'EveryNDays': 'varchar(max)', 'PatientID': 'varchar(max)', 'StartDate': 'varchar(max)', 'StartTime': 'varchar(max)', 'TaskID': 'varchar(max)', 'TermID': 'varchar(max)', 'TimestampCreated': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vTasks_ScheduleDay) y
