...
@model(
    table_description="Beställningsspec. En specifikation består av en unik kombination av flera men inte alltid alla av följande: analys, undersökning, rör, provmaterial. (MultiLabb analyskatalog)",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'AnalysisID': 'varchar(max)', 'ExaminationID': 'varchar(max)', 'IsAcute': 'varchar(max)', 'IsDrugInfoRequired': 'varchar(max)', 'IsLocalizationRequired': 'varchar(max)', 'IsRoutine': 'varchar(max)', 'NoOfLabels': 'varchar(max)', 'OrderRegistryFileName': 'varchar(max)', 'OrderableID': 'varchar(max)', 'SectionCode': 'varchar(max)', 'SpecimenID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TubeID': 'varchar(max)'},
