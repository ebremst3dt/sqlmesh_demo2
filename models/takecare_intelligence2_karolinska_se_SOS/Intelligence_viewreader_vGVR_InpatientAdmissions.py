...
@model(
    table_description="GVR inskrivningstransaktioner och korrigeringar av inskrivning, för slutenvården.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'AdmissionCode': 'varchar(max)', 'AdmissionFormCode': 'varchar(max)', 'CareConnection': 'varchar(max)', 'ClinicalPathwayNumber': 'varchar(max)', 'FileName': 'varchar(max)', 'IsEmergency': 'varchar(max)', 'ReferringCareUnit': 'varchar(max)', 'ReferringClinic': 'varchar(max)', 'ReferringHospital': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TransactionID': 'varchar(max)', 'TreatmentPeriod': 'varchar(max)'},
