...
@model(
    table_description="Uppgifter som registreras i DRG-formuläret i TakeCare",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CarePlanDocumentID': 'varchar(max)', 'DocumentID': 'varchar(max)', 'HasAdditionalMeasures': 'varchar(max)', 'ICUDays': 'varchar(max)', 'ICUPoints': 'varchar(max)', 'IsAutopsied': 'varchar(max)', 'KatzID': 'varchar(max)', 'OutlierLimit': 'varchar(max)', 'PatientID': 'varchar(max)', 'ProductCode': 'varchar(max)', 'ProductName': 'varchar(max)', 'ProductType': 'varchar(max)', 'ProductWeight': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'WeightAtBirth': 'varchar(max)'},
