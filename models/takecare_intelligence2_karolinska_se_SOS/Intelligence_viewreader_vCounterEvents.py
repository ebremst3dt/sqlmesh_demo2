...
@model(
    table_description="Kassahändelser/kassastatus från dagliga filer. En del av de senaste kassahändelserna kopieras över från en dags kassastatusfil till nästa dags, utom stängning av kassa. Det enda som skiljer dessa kopior från varandra och originalet är vilken fil de lästs från av Intelligence.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CompanyGroup': 'varchar(max)', 'Counter': 'varchar(max)', 'EventDate': 'varchar(max)', 'EventID': 'varchar(max)', 'EventTime': 'varchar(max)', 'FileNameDate': 'varchar(max)', 'InternalCounterCode': 'varchar(max)', 'ReportDate': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'UserID': 'varchar(max)'},
