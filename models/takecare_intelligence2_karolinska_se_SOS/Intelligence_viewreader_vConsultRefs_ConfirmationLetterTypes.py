...
@model(
    table_description="Info om de remissbekräftelsemallar som använts vid remissbekräftelse.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CareUnitID': 'varchar(max)', 'ConfirmedDateTime': 'varchar(max)', 'DocumentID': 'varchar(max)', 'OutputFormatID': 'varchar(max)', 'PatientID': 'varchar(max)', 'ReferralNotificationMethodID': 'varchar(max)', 'Row': 'varchar(max)', 'TemplateID': 'varchar(max)', 'TemplateName': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
