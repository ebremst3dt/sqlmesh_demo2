...
@model(
    table_description="Aktiviteten utförs enligt schema vissa veckodagar, en gång per dag.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'EndDate': 'varchar(max)', 'EndTime': 'varchar(max)', 'EveryNWeeks': 'varchar(max)', 'IsPerformedOnFridays': 'varchar(max)', 'IsPerformedOnMondays': 'varchar(max)', 'IsPerformedOnSaturdays': 'varchar(max)', 'IsPerformedOnSundays': 'varchar(max)', 'IsPerformedOnThursdays': 'varchar(max)', 'IsPerformedOnTuesdays': 'varchar(max)', 'IsPerformedOnWednesdays': 'varchar(max)', 'PatientID': 'varchar(max)', 'StartDate': 'varchar(max)', 'StartTime': 'varchar(max)', 'TaskID': 'varchar(max)', 'TermID': 'varchar(max)', 'TimestampCreated': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vTasks2_ScheduleWeek) y
