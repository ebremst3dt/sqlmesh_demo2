...
@model(
    table_description="Analyser som kan kopplas till en beställning (farmlab), nedlagd, användes till april 2010",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'ATC1': 'varchar(max)', 'ATC2': 'varchar(max)', 'ATC3': 'varchar(max)', 'Analysis': 'varchar(max)', 'AnalysisID': 'varchar(max)', 'IsLatestDoseRequired': 'varchar(max)', 'IsPatientCreatinineRequired': 'varchar(max)', 'IsPatientLengthRequired': 'varchar(max)', 'IsPatientSmokerRequired': 'varchar(max)', 'IsPatientWeightRequired': 'varchar(max)', 'TestCauseCode': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TubeID': 'varchar(max)'},
