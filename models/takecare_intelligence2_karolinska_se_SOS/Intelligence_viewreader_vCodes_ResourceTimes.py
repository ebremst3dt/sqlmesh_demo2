...
@model(
    table_description="Definierade tider för en resurs, en tid kan t.ex vara "Rond"",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'BookableDaysAhead': 'varchar(max)', 'CareUnitID': 'varchar(max)', 'FromDate': 'varchar(max)', 'FromTime': 'varchar(max)', 'ResourceID': 'varchar(max)', 'SlotLength': 'varchar(max)', 'TimeTypeID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ToDate': 'varchar(max)', 'ToTime': 'varchar(max)', 'WeekdayID': 'varchar(max)'},
