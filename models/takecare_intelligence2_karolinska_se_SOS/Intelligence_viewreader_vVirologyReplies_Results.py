...
@model(
    table_description="Resultat från analyser för virologi. Ett svar kan innehålla flera resultat med ett eget utlåtande och svarsdatum. Ett resultat kan omfatta flera analyser.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'DocumentID': 'varchar(max)', 'LID': 'varchar(max)', 'LabNotes': 'varchar(max)', 'LabResponsibleDoctor': 'varchar(max)', 'PatientID': 'varchar(max)', 'ReplyType': 'varchar(max)', 'ResultComment': 'varchar(max)', 'ResultText': 'varchar(max)', 'ResultTimestamp': 'varchar(max)', 'Row': 'varchar(max)', 'SectionNotes': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
