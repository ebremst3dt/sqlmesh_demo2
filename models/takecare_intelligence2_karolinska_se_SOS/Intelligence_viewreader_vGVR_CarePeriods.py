...
@model(
    table_description="GVR-transaktioner som hör ihop med en vårdperiod. Om de två fälten (GVRs interna nyckelfält) PatientID och CarePeriodCode har samma värde för flera Vårdperiodtransaktioner så hör transaktionerna ihop.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CarePeriodCode': 'varchar(max)', 'EconomicalKombika': 'varchar(max)', 'EventDatetime': 'varchar(max)', 'FileName': 'varchar(max)', 'IsCancellation': 'varchar(max)', 'IsCorrection': 'varchar(max)', 'PatientID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TransactionID': 'varchar(max)'},
