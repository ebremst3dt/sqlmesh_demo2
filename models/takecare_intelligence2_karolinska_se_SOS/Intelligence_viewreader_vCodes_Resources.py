...
@model(
    table_description="En resurs kan vara t.ex. en person, ett rum, eller en befattning. En bokning görs för en viss resurs ett visst datum på en viss tid. Resurser lagras inte med versionshantering, däremot tas resurser aldrig bort. Varje vårdenhet definierar själv sina resurser.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CareProviderID': 'varchar(max)', 'CareUnitID': 'varchar(max)', 'Name': 'varchar(max)', 'ResourceID': 'varchar(max)', 'ResourceTypeID': 'varchar(max)', 'ScheduleMode': 'varchar(max)', 'ShareStatus': 'varchar(max)', 'ShortName': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidFromDate': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
