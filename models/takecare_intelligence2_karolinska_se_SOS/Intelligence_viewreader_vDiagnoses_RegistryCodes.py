...
@model(
    table_description="Diagnoser som användaren kan söka efter i diagnosregister, primärvårdsdiagnoser, orsaksregister eller ATC-register. Maximalt fyra diagnosrader kan tillhöra en diagnosversion.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'Code': 'varchar(max)', 'CodeTable': 'varchar(max)', 'DocumentID': 'varchar(max)', 'PatientID': 'varchar(max)', 'Text': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Type': 'varchar(max)', 'Version': 'varchar(max)'},
