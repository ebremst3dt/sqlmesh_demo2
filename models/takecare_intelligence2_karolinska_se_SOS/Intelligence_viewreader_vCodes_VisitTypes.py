...
@model(
    table_description="Besökstyper för bokningar och kassa. En uppsättning koder per företag.",
    columns={'_data_modified_utc': 'datetime', '_metadata_modified_utc': 'datetime', 'CompanyCode': 'varchar(max)', 'CompanyID': 'varchar(max)', 'CountryID': 'varchar(max)', 'CountyID': 'varchar(max)', 'FirstVisitPossible': 'varchar(max)', 'IsAcknowledged': 'varchar(max)', 'IsAutomaticallyApproved': 'varchar(max)', 'IsReferralTariffToBeUsed': 'varchar(max)', 'Name': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidFromDate': 'varchar(max)', 'ValidThroughDate': 'varchar(max)', 'VisitTypeCategoryID': 'varchar(max)', 'VisitTypeCode': 'varchar(max)', 'VisitTypeID': 'varchar(max)'},
