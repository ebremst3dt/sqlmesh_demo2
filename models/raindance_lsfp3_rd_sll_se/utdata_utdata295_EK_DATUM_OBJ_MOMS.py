...
@model(
    columns={'_data_modified_utc': 'date', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'MOMS_DATUM_FOM': 'varchar(max)', 'MOMS_DATUM_TOM': 'varchar(max)', 'MOMS_GILTIG_FOM': 'varchar(max)', 'MOMS_GILTIG_TOM': 'varchar(max)', 'MOMS_ID': 'varchar(max)', 'MOMS_ID_TEXT': 'varchar(max)', 'MOMS_PASSIV': 'varchar(max)', 'MOMS_TEXT': 'varchar(max)'},
    kind=dict(
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM utdata.utdata295.EK_DATUM_OBJ_MOMS) y
