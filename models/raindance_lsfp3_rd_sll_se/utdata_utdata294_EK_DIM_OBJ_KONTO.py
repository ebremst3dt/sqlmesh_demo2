...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CONVERT(varchar(max), ITGRP_GILTIG_FOM, 126) AS itgrp_giltig_fom,
		CONVERT(varchar(max), ITGRP_GILTIG_TOM, 126) AS itgrp_giltig_tom,
		CAST(ITGRP_ID AS VARCHAR(MAX)) AS itgrp_id,
		CAST(ITGRP_ID_TEXT AS VARCHAR(MAX)) AS itgrp_id_text,
		CAST(ITGRP_PASSIV AS VARCHAR(MAX)) AS itgrp_passiv,
		CAST(ITGRP_TEXT AS VARCHAR(MAX)) AS itgrp_text,
