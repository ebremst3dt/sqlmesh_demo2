...
@model(
    columns={'_data_modified_utc': 'date', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AR': 'varchar(max)', 'AR_TEXT': 'varchar(max)', 'ATTESTDATUM1': 'varchar(max)', 'ATTESTDATUM1_TEXT': 'varchar(max)', 'BOKFORINGSAR': 'varchar(max)', 'BOKFORINGSARSLUT': 'varchar(max)', 'BOKFORINGSAR_TEXT': 'varchar(max)', 'DAG': 'varchar(max)', 'DAG_TEXT': 'varchar(max)', 'DATUM6B_TEXT': 'varchar(max)', 'DATUM6_TEXT': 'varchar(max)', 'DATUM8_TEXT': 'varchar(max)', 'KVARTAL': 'varchar(max)', 'KVARTALNR': 'varchar(max)', 'KVARTALNR_TEXT': 'varchar(max)', 'KVARTAL_TEXT': 'varchar(max)', 'MANAD': 'varchar(max)', 'MANADNR': 'varchar(max)', 'MANADNR_TEXT': 'varchar(max)', 'MANADSNAMN': 'varchar(max)', 'MANAD_TEXT': 'varchar(max)', 'PERIOD': 'varchar(max)', 'PERIOD10_TEXT': 'varchar(max)', 'PERIOD4_TEXT': 'varchar(max)', 'PERIOD6B_TEXT': 'varchar(max)', 'PERIOD6_TEXT': 'varchar(max)', 'PERIOD7_TEXT': 'varchar(max)', 'PERIOD8_TEXT': 'varchar(max)', 'PERIODSLUT': 'varchar(max)', 'PERIODSTATUS': 'varchar(max)', 'PERIODSTATUS_TEXT': 'varchar(max)', 'PERIOD_TEXT': 'varchar(max)', 'TERTIAL': 'varchar(max)', 'TERTIALNR': 'varchar(max)', 'TERTIALNR_TEXT': 'varchar(max)', 'TERTIAL_TEXT': 'varchar(max)', 'VECKA': 'varchar(max)', 'VECKA_TEXT': 'varchar(max)', 'VECKODAG': 'varchar(max)', 'VECKODAG_TEXT': 'varchar(max)', 'VECKONR': 'varchar(max)', 'VECKONR_TEXT': 'varchar(max)', 'VECKO_TEXT': 'varchar(max)'},
    kind=dict(
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST(ATTESTDATUM1_TEXT AS VARCHAR(MAX)) AS attestdatum1_text,
