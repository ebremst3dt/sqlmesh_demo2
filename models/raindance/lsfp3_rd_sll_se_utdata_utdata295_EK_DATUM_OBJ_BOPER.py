
import typing as t
from datetime import datetime
import pandas as pd
from sqlmesh import ExecutionContext, model
from sqlmesh.core.model.kind import ModelKindName
from models.mssql import read


@model(
    columns={'BOPER_DATUM_FOM': 'datetime',
 'BOPER_DATUM_TOM': 'datetime',
 'BOPER_GILTIG_FOM': 'datetime',
 'BOPER_GILTIG_TOM': 'datetime',
 'BOPER_ID': 'varchar(1)',
 'BOPER_ID_TEXT': 'varchar(32)',
 'BOPER_PASSIV': 'bit',
 'BOPER_TEXT': 'varchar(30)'},
    kind=ModelKindName.FULL,
    cron="@daily"
)
def execute(
    context: ExecutionContext,
    start: datetime,
    end: datetime,
    execution_time: datetime,
    **kwargs: t.Any,
) -> pd.DataFrame:
    query = """
	SELECT top 1000
 		CAST(BOPER_DATUM_FOM AS VARCHAR(MAX)) AS boper_datum_fom,
		CAST(BOPER_DATUM_TOM AS VARCHAR(MAX)) AS boper_datum_tom,
		CAST(BOPER_GILTIG_FOM AS VARCHAR(MAX)) AS boper_giltig_fom,
		CAST(BOPER_GILTIG_TOM AS VARCHAR(MAX)) AS boper_giltig_tom,
		CAST(BOPER_ID AS VARCHAR(MAX)) AS boper_id,
		CAST(BOPER_ID_TEXT AS VARCHAR(MAX)) AS boper_id_text,
		CAST(BOPER_PASSIV AS VARCHAR(MAX)) AS boper_passiv,
		CAST(BOPER_TEXT AS VARCHAR(MAX)) AS boper_text 
	FROM utdata.utdata295.EK_DATUM_OBJ_BOPER
	"""
    return read(query=query, server_url="lsfp3.rd.sll.se")
