...
@model(
    columns={'_data_modified_utc': 'date', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'DIV_GILTIG_FOM': 'varchar(max)', 'DIV_GILTIG_TOM': 'varchar(max)', 'DIV_ID': 'varchar(max)', 'DIV_ID_TEXT': 'varchar(max)', 'DIV_PASSIV': 'varchar(max)', 'DIV_TEXT': 'varchar(max)', 'KLINIK_GILTIG_FOM': 'varchar(max)', 'KLINIK_GILTIG_TOM': 'varchar(max)', 'KLINIK_ID': 'varchar(max)', 'KLINIK_ID_TEXT': 'varchar(max)', 'KLINIK_PASSIV': 'varchar(max)', 'KLINIK_TEXT': 'varchar(max)'},
    kind=dict(
