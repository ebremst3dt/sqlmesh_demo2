...
@model(
    columns={'_data_modified_utc': 'date', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'ADR1': 'varchar(max)', 'ADR2': 'varchar(max)', 'ATTRIBUTE_ACTORID': 'varchar(max)', 'ATTRIBUTE_EAN': 'varchar(max)', 'BETSPARR': 'varchar(max)', 'BGNR': 'varchar(max)', 'DUMMY1': 'varchar(max)', 'ELEKTRONISK_KOMMUNIKATION': 'varchar(max)', 'ENGANG': 'varchar(max)', 'FRI1': 'varchar(max)', 'FRI2': 'varchar(max)', 'FRI3': 'varchar(max)', 'FRI4': 'varchar(max)', 'GRUPP': 'varchar(max)', 'KREDITLIMIT': 'varchar(max)', 'KUNDID': 'varchar(max)', 'KUNDID2': 'varchar(max)', 'KUNDID_ID_TEXT': 'varchar(max)', 'KUNDID_TEXT': 'varchar(max)', 'KUND_EREF': 'varchar(max)', 'KUND_TAB_AVTTYP': 'varchar(max)', 'KUND_TAB_BEHÄND': 'varchar(max)', 'KUND_TAB_BETP': 'varchar(max)', 'KUND_TAB_BETV': 'varchar(max)', 'KUND_TAB_EFAK': 'varchar(max)', 'KUND_TAB_GLN': 'varchar(max)', 'KUND_TAB_INAVSE': 'varchar(max)', 'KUND_TAB_INKASS': 'varchar(max)', 'KUND_TAB_KRAVGR': 'varchar(max)', 'KUND_TAB_KST': 'varchar(max)', 'KUND_TAB_LAND': 'varchar(max)', 'KUND_TAB_LAYOUT': 'varchar(max)', 'KUND_TAB_MOMS': 'varchar(max)', 'KUND_TAB_MOTP': 'varchar(max)', 'KUND_TAB_RDEB': 'varchar(max)', 'KUND_TAB_RESK': 'varchar(max)', 'KUND_TAB_RTYP': 'varchar(max)', 'KUND_TAB_SI': 'varchar(max)', 'KUND_TAB_SPRÅK': 'varchar(max)', 'KUND_TAB_VAKR': 'varchar(max)', 'KUND_TAB_VATNR': 'varchar(max)', 'KUND_VREF': 'varchar(max)', 'LAND': 'varchar(max)', 'MOMSREG': 'varchar(max)', 'MSG_INVOIC_FORMAT': 'varchar(max)', 'MSG_INVOIC_PA': 'varchar(max)', 'MSG_INVOIC_TRANSPORT': 'varchar(max)', 'MSG_INVOIC_VAG': 'varchar(max)', 'NAMN2': 'varchar(max)', 'OMSATTNING': 'varchar(max)', 'OMSATTNINGSAR': 'varchar(max)', 'OMSATTNING_FG': 'varchar(max)', 'ORGNR': 'varchar(max)', 'ORT': 'varchar(max)', 'PASSIV': 'varchar(max)', 'PGNR': 'varchar(max)', 'SALDO': 'varchar(max)', 'SOKBEGR': 'varchar(max)', 'TELNR': 'varchar(max)'},
    kind=dict(
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST(KUND_TAB_EFAK AS VARCHAR(MAX)) AS kund_tab_efak,
		CAST(KUND_TAB_GLN AS VARCHAR(MAX)) AS kund_tab_gln,
		CAST(KUND_TAB_INAVSE AS VARCHAR(MAX)) AS kund_tab_inavse,
		CAST(KUND_TAB_INKASS AS VARCHAR(MAX)) AS kund_tab_inkass,
		CAST(KUND_TAB_KRAVGR AS VARCHAR(MAX)) AS kund_tab_kravgr,
		CAST(KUND_TAB_KST AS VARCHAR(MAX)) AS kund_tab_kst,
		CAST(KUND_TAB_LAND AS VARCHAR(MAX)) AS kund_tab_land,
		CAST(KUND_TAB_LAYOUT AS VARCHAR(MAX)) AS kund_tab_layout,
		...
		CAST(KUND_TAB_RESK AS VARCHAR(MAX)) AS kund_tab_resk,
		CAST(KUND_TAB_RTYP AS VARCHAR(MAX)) AS kund_tab_rtyp,
		CAST(KUND_TAB_SI AS VARCHAR(MAX)) AS kund_tab_si,
		CAST(KUND_TAB_SPRÅK AS VARCHAR(MAX)) AS kund_tab_språk,
		CAST(KUND_TAB_VAKR AS VARCHAR(MAX)) AS kund_tab_vakr,
		CAST(KUND_TAB_VATNR AS VARCHAR(MAX)) AS kund_tab_vatnr,
