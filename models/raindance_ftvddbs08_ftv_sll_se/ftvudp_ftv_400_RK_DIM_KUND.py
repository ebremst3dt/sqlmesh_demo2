...
@model(
    columns={'_data_modified_utc': 'date', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'ADR1': 'varchar(max)', 'ADR2': 'varchar(max)', 'ATTRIBUTE_ACTORID': 'varchar(max)', 'ATTRIBUTE_EAN': 'varchar(max)', 'BETSPARR': 'varchar(max)', 'BGNR': 'varchar(max)', 'DUMMY1': 'varchar(max)', 'ELEKTRONISK_KOMMUNIKATION': 'varchar(max)', 'ENGANG': 'varchar(max)', 'FRI1': 'varchar(max)', 'FRI2': 'varchar(max)', 'FRI3': 'varchar(max)', 'FRI4': 'varchar(max)', 'GRUPP': 'varchar(max)', 'KREDITLIMIT': 'varchar(max)', 'KUNDID': 'varchar(max)', 'KUNDID2': 'varchar(max)', 'KUNDID_ID_TEXT': 'varchar(max)', 'KUNDID_TEXT': 'varchar(max)', 'KUND_EREF': 'varchar(max)', 'KUND_TAB_AF': 'varchar(max)', 'KUND_TAB_AGF': 'varchar(max)', 'KUND_TAB_AVBET': 'varchar(max)', 'KUND_TAB_BEHÄND': 'varchar(max)', 'KUND_TAB_BETP': 'varchar(max)', 'KUND_TAB_BETV': 'varchar(max)', 'KUND_TAB_DEBAVB': 'varchar(max)', 'KUND_TAB_DEBBP': 'varchar(max)', 'KUND_TAB_DIV': 'varchar(max)', 'KUND_TAB_ENHET': 'varchar(max)', 'KUND_TAB_FELKOD': 'varchar(max)', 'KUND_TAB_INK': 'varchar(max)', 'KUND_TAB_KAT': 'varchar(max)', 'KUND_TAB_KRAVGR': 'varchar(max)', 'KUND_TAB_LÖPNR': 'varchar(max)', 'KUND_TAB_MOMS': 'varchar(max)', 'KUND_TAB_MOTP': 'varchar(max)', 'KUND_TAB_PERSNR': 'varchar(max)', 'KUND_TAB_RDEB': 'varchar(max)', 'KUND_TAB_RÄNTGR': 'varchar(max)', 'KUND_TAB_UBF': 'varchar(max)', 'KUND_VREF': 'varchar(max)', 'LAND': 'varchar(max)', 'MOMSREG': 'varchar(max)', 'MSG_INVOIC_FORMAT': 'varchar(max)', 'MSG_INVOIC_PA': 'varchar(max)', 'MSG_INVOIC_TRANSPORT': 'varchar(max)', 'MSG_INVOIC_VAG': 'varchar(max)', 'NAMN2': 'varchar(max)', 'OMSATTNING': 'varchar(max)', 'OMSATTNINGSAR': 'varchar(max)', 'OMSATTNING_FG': 'varchar(max)', 'ORGNR': 'varchar(max)', 'ORT': 'varchar(max)', 'PASSIV': 'varchar(max)', 'PGNR': 'varchar(max)', 'SALDO': 'varchar(max)', 'SOKBEGR': 'varchar(max)', 'TELNR': 'varchar(max)'},
    kind=dict(
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST(KUND_TAB_AF AS VARCHAR(MAX)) AS kund_tab_af,
		CAST(KUND_TAB_AGF AS VARCHAR(MAX)) AS kund_tab_agf,
		CAST(KUND_TAB_AVBET AS VARCHAR(MAX)) AS kund_tab_avbet,
		...
		CAST(KUND_TAB_DEBAVB AS VARCHAR(MAX)) AS kund_tab_debavb,
		CAST(KUND_TAB_DEBBP AS VARCHAR(MAX)) AS kund_tab_debbp,
		CAST(KUND_TAB_DIV AS VARCHAR(MAX)) AS kund_tab_div,
		CAST(KUND_TAB_ENHET AS VARCHAR(MAX)) AS kund_tab_enhet,
		CAST(KUND_TAB_FELKOD AS VARCHAR(MAX)) AS kund_tab_felkod,
		CAST(KUND_TAB_INK AS VARCHAR(MAX)) AS kund_tab_ink,
		CAST(KUND_TAB_KAT AS VARCHAR(MAX)) AS kund_tab_kat,
		CAST(KUND_TAB_KRAVGR AS VARCHAR(MAX)) AS kund_tab_kravgr,
		CAST(KUND_TAB_LÖPNR AS VARCHAR(MAX)) AS kund_tab_löpnr,
		CAST(KUND_TAB_MOMS AS VARCHAR(MAX)) AS kund_tab_moms,
		CAST(KUND_TAB_MOTP AS VARCHAR(MAX)) AS kund_tab_motp,
		CAST(KUND_TAB_PERSNR AS VARCHAR(MAX)) AS kund_tab_persnr,
		CAST(KUND_TAB_RDEB AS VARCHAR(MAX)) AS kund_tab_rdeb,
		CAST(KUND_TAB_RÄNTGR AS VARCHAR(MAX)) AS kund_tab_räntgr,
		CAST(KUND_TAB_UBF AS VARCHAR(MAX)) AS kund_tab_ubf,
