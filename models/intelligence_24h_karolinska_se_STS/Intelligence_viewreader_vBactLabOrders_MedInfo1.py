...
@model(
    description="Kompletterande uppgifter (tidigare medicinsk information). Skapades av det gränssnitt som tidigare användes på Huddinge Sjukhus.",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AntibioticsTherapy': 'varchar(max)', 'DiscontinuedDate': 'varchar(max)', 'DocumentID': 'varchar(max)', 'EnteredDate': 'varchar(max)', 'PatientID': 'varchar(max)', 'Row': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
