...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'DocumentID': 'varchar(max)', 'IsRequired': 'varchar(max)', 'MedInfoID': 'varchar(max)', 'PatientID': 'varchar(max)', 'Row': 'varchar(max)', 'TextLabel': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Type': 'varchar(max)', 'Unit': 'varchar(max)', 'Version': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vMultiLabOrders_MedInfo) y
