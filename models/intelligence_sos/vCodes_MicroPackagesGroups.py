...
@model(
    description="Vilka paket som ingår i varje grupp. (Mikrolabb analyskatalog)",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'GroupID': 'varchar(max)', 'OrderRegistryFileName': 'varchar(max)', 'PackageID': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['GroupID', 'OrderRegistryFileName', 'PackageID']
    ),
    cron="@daily",
    start=start,
    enabled=True
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vCodes_MicroPackagesGroups) y
