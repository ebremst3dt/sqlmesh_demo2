...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'Name': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidThroughDate': 'varchar(max)', 'VisitTypeCategoryID': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['VisitTypeCategoryID']
    ),
    cron="@daily",
    start=start,
    enabled=True
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vCodes_VisitTypeCategories) y
