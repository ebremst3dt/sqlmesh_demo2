...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'ProfileID': 'varchar(max)', 'RoleID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'UserID': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['ProfileID', 'RoleID', 'UserID', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vUsers_RolesProfiles) y
