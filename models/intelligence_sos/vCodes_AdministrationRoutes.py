...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AdministrationRouteID': 'varchar(max)', 'DoseForm': 'varchar(max)', 'IsInfusion': 'varchar(max)', 'IsOxygenTreatment': 'varchar(max)', 'Name': 'varchar(max)', 'ShortName': 'varchar(max)', 'SimpleName': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['AdministrationRouteID']
    ),
    cron="@daily",
    start=start,
    enabled=True
