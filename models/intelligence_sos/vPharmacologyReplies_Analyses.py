...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'Analysis': 'varchar(max)', 'AnalysisComment': 'varchar(max)', 'AnalysisID': 'varchar(max)', 'AnalysisRow': 'varchar(max)', 'DocumentID': 'varchar(max)', 'GroupRow': 'varchar(max)', 'IsDeviating': 'varchar(max)', 'PatientID': 'varchar(max)', 'Priority': 'varchar(max)', 'ReferenceArea1': 'varchar(max)', 'ReferenceArea2': 'varchar(max)', 'ReplacementValue': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Unit': 'varchar(max)', 'Value': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['AnalysisRow', 'DocumentID', 'GroupRow', 'PatientID', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
