...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AnalysisID': 'varchar(max)', 'DocumentID': 'varchar(max)', 'LID': 'varchar(max)', 'PatientID': 'varchar(max)', 'ResultRow': 'varchar(max)', 'Row': 'varchar(max)', 'SamplingDate': 'varchar(max)', 'Specimen': 'varchar(max)', 'TestResult': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['AnalysisID', 'DocumentID', 'PatientID', 'ResultRow', 'Row', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
