...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'DocumentID': 'varchar(max)', 'Exam': 'varchar(max)', 'ExamDateTime': 'varchar(max)', 'ExamID': 'varchar(max)', 'MethodCode': 'varchar(max)', 'PatientID': 'varchar(max)', 'Row': 'varchar(max)', 'Template': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['DocumentID', 'ExamID', 'PatientID', 'Row', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
