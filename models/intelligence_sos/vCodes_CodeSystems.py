...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'CodeSystemID': 'varchar(max)', 'InternalCode': 'varchar(max)', 'Name': 'varchar(max)', 'OID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST([InternalCode] AS VARCHAR(MAX)) AS [InternalCode],
		CAST([Name] AS VARCHAR(MAX)) AS [Name],
		CAST([OID] AS VARCHAR(MAX)) AS [OID],
		CONVERT(varchar(max), [TimestampRead], 126) AS [TimestampRead],
		CONVERT(varchar(max), [ValidThroughDate], 126) AS [ValidThroughDate] 
	FROM Intelligence.viewreader.vCodes_CodeSystems) y
