...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AppNotificationTypeID': 'varchar(max)', 'Name': 'varchar(max)', 'ReferralNotificationMethodID': 'varchar(max)', 'ShortName': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['ReferralNotificationMethodID']
    ),
    cron="@daily",
    start=start,
    enabled=True
