...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AnalysisRow': 'varchar(max)', 'DocumentID': 'varchar(max)', 'GroupRow': 'varchar(max)', 'IsDeviating': 'varchar(max)', 'OccasionRow': 'varchar(max)', 'PatientID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Value': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['AnalysisRow', 'DocumentID', 'GroupRow', 'OccasionRow', 'PatientID', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
