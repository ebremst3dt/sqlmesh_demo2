...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'CompositeID': 'varchar(max)', 'CountyID': 'varchar(max)', 'MunicipalityID': 'varchar(max)', 'ParishID': 'varchar(max)', 'ParishName': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['CountyID', 'MunicipalityID', 'ParishID']
    ),
    cron="@daily",
    start=start,
    enabled=True
