...
@model(
    description="Angivna åtgärdskoder vid KÖKS (KVÅ-koder)",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'ActionCode': 'varchar(max)', 'ActionDate': 'varchar(max)', 'ActionID': 'varchar(max)', 'DiagnosisID': 'varchar(max)', 'DocumentID': 'varchar(max)', 'PatientID': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['ActionID', 'DocumentID', 'PatientID']
    ),
    cron="@daily",
    start=start,
    enabled=True
