...
@model(
    description="En tidstyp anger vilken typ av aktivitet som en tid är reserverad för och kan vara t.ex 'Rond' eller 'Mottagning'. Tidstyper definieras per vårdenhet. Vissa tidstyper existerar bara i TakeCares gränssnitt.",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'CareUnitID': 'varchar(max)', 'ColourCodeBlue': 'varchar(max)', 'ColourCodeGreen': 'varchar(max)', 'ColourCodeRed': 'varchar(max)', 'HasSelfCheckIn': 'varchar(max)', 'IsBookable': 'varchar(max)', 'IsHiddenForCounter': 'varchar(max)', 'IsHiddenForPatient': 'varchar(max)', 'IsWebBookable': 'varchar(max)', 'IsWebCancellable': 'varchar(max)', 'IsWebRebookable': 'varchar(max)', 'Name': 'varchar(max)', 'Purpose': 'varchar(max)', 'TimeTypeID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'WebName': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['CareUnitID', 'TimeTypeID']
    ),
    cron="@daily",
    start=start,
    enabled=True
