...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'ColumnTermID': 'varchar(max)', 'TermID': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['ColumnTermID', 'TermID']
    ),
    cron="@daily",
    start=start,
    enabled=True
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST([ColumnTermID] AS VARCHAR(MAX)) AS [ColumnTermID],
		CAST([TermID] AS VARCHAR(MAX)) AS [TermID],
		CONVERT(varchar(max), [TimestampRead], 126) AS [TimestampRead] 
	FROM Intelligence.viewreader.vCodes_TermContainerColumns) y
