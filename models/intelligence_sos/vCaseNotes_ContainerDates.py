...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'ColumnHeadingTermID': 'varchar(max)', 'Date': 'varchar(max)', 'DocumentID': 'varchar(max)', 'KeywordTermID': 'varchar(max)', 'PatientID': 'varchar(max)', 'Row': 'varchar(max)', 'Time': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['ColumnHeadingTermID', 'DocumentID', 'KeywordTermID', 'PatientID', 'Row', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
...
def execute(
    ...
    **kwargs: t.Any,
	...
	FROM Intelligence.viewreader.vCaseNotes_ContainerDates) y
