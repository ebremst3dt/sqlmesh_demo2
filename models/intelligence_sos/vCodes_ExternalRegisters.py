...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'Description': 'varchar(max)', 'ExternalRegisterID': 'varchar(max)', 'File': 'varchar(max)', 'IsUsedInCaseNotes': 'varchar(max)', 'Origin': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST([File] AS VARCHAR(MAX)) AS [File],
		CAST([IsUsedInCaseNotes] AS VARCHAR(MAX)) AS [IsUsedInCaseNotes],
		CAST([Origin] AS VARCHAR(MAX)) AS [Origin],
		CONVERT(varchar(max), [TimestampRead], 126) AS [TimestampRead],
		CONVERT(varchar(max), [ValidThroughDate], 126) AS [ValidThroughDate] 
	FROM Intelligence.viewreader.vCodes_ExternalRegisters) y
