...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AddonProductID': 'varchar(max)', 'Amount': 'varchar(max)', 'DocumentID': 'varchar(max)', 'PatientID': 'varchar(max)', 'Price': 'varchar(max)', 'ProductCode': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'Weight': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['AddonProductID', 'DocumentID', 'PatientID']
    ),
    cron="@daily",
    start=start,
    enabled=True
