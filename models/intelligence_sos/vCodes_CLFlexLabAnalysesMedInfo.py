...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AnalysisID': 'varchar(max)', 'IsMedInfoRequired': 'varchar(max)', 'MedInfoID': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['AnalysisID', 'MedInfoID']
    ),
    cron="@daily",
    start=start,
    enabled=True
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST([IsMedInfoRequired] AS VARCHAR(MAX)) AS [IsMedInfoRequired],
		CAST([MedInfoID] AS VARCHAR(MAX)) AS [MedInfoID],
		CONVERT(varchar(max), [TimestampRead], 126) AS [TimestampRead] 
	FROM Intelligence.viewreader.vCodes_CLFlexLabAnalysesMedInfo) y
