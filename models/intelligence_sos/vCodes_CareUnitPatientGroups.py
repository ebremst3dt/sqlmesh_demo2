...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'CreatedAtCareUnitID': 'varchar(max)', 'Group': 'varchar(max)', 'GroupRow': 'varchar(max)', 'IsAutoRemoved': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['CreatedAtCareUnitID', 'GroupRow']
    ),
    cron="@daily",
    start=start,
    enabled=True
