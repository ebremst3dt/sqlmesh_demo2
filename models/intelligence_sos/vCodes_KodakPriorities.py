...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'LabOrderSettingsID': 'varchar(max)', 'PriorityCode': 'varchar(max)', 'PriorityID': 'varchar(max)', 'PriorityName': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['LabOrderSettingsID', 'PriorityID']
    ),
    cron="@daily",
    start=start,
    enabled=True
