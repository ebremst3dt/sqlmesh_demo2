...
@model(
    description="Ekonomiska enheter/ fakturamottagare",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'AccountingUnitKombika': 'varchar(max)', 'AccountingUnitName': 'varchar(max)', 'CareUnitID': 'varchar(max)', 'TimestampRead': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=5000,
        unique_key=['AccountingUnitKombika', 'AccountingUnitName', 'CareUnitID']
    ),
    cron="@daily",
    start=start,
    enabled=True
