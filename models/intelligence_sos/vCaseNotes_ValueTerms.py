...
@model(
    description="Vissa sökord kan ha ett eller flera värden i form av värdetermer.",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'DocumentID': 'varchar(max)', 'KeywordTermID': 'varchar(max)', 'PatientID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValueTermID': 'varchar(max)', 'Version': 'varchar(max)'},
    ...
    kind=dict(
        ...
        batch_size=30,
        unique_key=['DocumentID', 'KeywordTermID', 'PatientID', 'ValueTermID', 'Version']
    ),
    cron="@daily",
    start=start,
    enabled=True
