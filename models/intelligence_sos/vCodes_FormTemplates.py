...
@model(
    description="Formulärdefinitioner (Design av formulär och blanketter)",
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'Description': 'varchar(max)', 'EDISettingID': 'varchar(max)', 'FormName': 'varchar(max)', 'FormTemplateID': 'varchar(max)', 'HasToBeSent': 'varchar(max)', 'HasToBeSigned': 'varchar(max)', 'IsGroupEditable': 'varchar(max)', 'IsLinkedDocsEnabled': 'varchar(max)', 'IsPublished': 'varchar(max)', 'IsWatchEnabled': 'varchar(max)', 'LatestVersion': 'varchar(max)', 'SavedBy': 'varchar(max)', 'SavedByUserID': 'varchar(max)', 'StorageType': 'varchar(max)', 'TimestampCreated': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'TimestampSaved': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
