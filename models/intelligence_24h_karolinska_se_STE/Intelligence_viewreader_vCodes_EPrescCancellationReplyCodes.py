...
@model(
    ...
    columns={'_data_modified_utc': 'datetime2', '_metadata_modified_utc': 'datetime2', '_source': 'varchar(max)', 'Description': 'varchar(max)', 'ReplyID': 'varchar(max)', 'TimestampRead': 'varchar(max)', 'ValidThroughDate': 'varchar(max)'},
...
def execute(
    ...
    **kwargs: t.Any,
	...
	SELECT * FROM (SELECT 
		...
		CAST([ReplyID] AS VARCHAR(MAX)) AS [ReplyID],
		CONVERT(varchar(max), [TimestampRead], 126) AS [TimestampRead],
		CONVERT(varchar(max), [ValidThroughDate], 126) AS [ValidThroughDate] 
	FROM Intelligence.viewreader.vCodes_EPrescCancellationReplyCodes) y
